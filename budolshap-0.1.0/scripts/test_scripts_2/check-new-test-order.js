...
async function main() {
    const bookingId = '3378364475488901307';
    ...
    if (order.shipping?.driver) {
        ...
        console.log('\n❌ No driver info in database yet');
    }

    if (order.shipping?.timeline) {
        console.log('\n📋 Timeline Events:');
        order.shipping.timeline.forEach(event => {
            console.log(`   - ${event.event} (${event.status}) at ${event.timestamp}`);
        });
    }
}
