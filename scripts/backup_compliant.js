...
// Backup to D:\IT Projects\budolEcosystem\backup-db\db-YYYY-MM-DD
const ROOT_DIR = path.resolve(__dirname, '..');
const BACKUP_ROOT = path.join(ROOT_DIR, 'backup-db');
const DATE_STR = new Date().toISOString().split('T')[0];
const BACKUP_DIR = path.join(BACKUP_ROOT, `db-${DATE_STR}`);
...
async function backup() {
    console.log(`Starting SQL backups to ${BACKUP_DIR}...`);
