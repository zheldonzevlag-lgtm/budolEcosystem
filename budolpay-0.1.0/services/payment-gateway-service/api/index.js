...
if (IS_VERCEL) {
  console.log('[Payment-Gateway] 🛡️ Vercel Environment Detected. Respecting Environment Variables.');
}
...
// --- INLINED DATABASE CLIENT & AUDIT LOG (v27.3 Self-Contained) ---
...
const createCentralizedAuditLog = async (data) => {
  try {
    const auditServiceUrl = process.env.AUDIT_SERVICE_URL || `http://${process.env.LOCAL_IP || 'localhost'}:8001`;
    await axios.post(`${auditServiceUrl}/logs`, data, { timeout: 1000 }).catch(() => {});
  } catch (e) {
    console.warn('[Self-Contained Gateway] Log failed:', e.message);
  }
};
