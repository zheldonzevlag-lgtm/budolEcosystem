...
import { LogOut, User, ChevronUp, ChevronDown, ChevronRight, Settings } from 'lucide-react';
...
export default function Sidebar() {
  ...
  const [showSettingsSubmenu, setShowSettingsSubmenu] = useState(pathname.startsWith('/settings'));
            ...
            <div className="relative">
              <button 
                ...
                className={`w-full flex items-center justify-between px-4 py-1.5 text-[13px] font-medium transition-all rounded-md group ${
                  pathname.startsWith('/settings') ? 'text-white bg-white/5' : 'text-white hover:bg-white/5'
                }`}
              >
                ...
                {showSettingsSubmenu ? <ChevronDown className="w-3 h-3 text-slate-500" /> : <ChevronRight className="w-3 h-3 text-slate-500" />}
              </button>
              
              {showSettingsSubmenu && (
            <div className="mt-1 ml-4 pl-4 border-l border-slate-800 space-y-0.5 animate-in fade-in slide-in-from-top-1 duration-200">
              <NavItem href="/settings#system" label="Environment Variables" active={pathname === '/settings' && !hash} isSubmenu />
              <NavItem href="/settings#payment" label="Payment Gateways" active={hash === '#payment'} isSubmenu />
              <NavItem href="/settings#realtime" label="Realtime Settings" active={hash === '#realtime'} isSubmenu />
              <NavItem href="/settings#notification" label="Notification Settings" active={hash === '#notification'} isSubmenu />
              <NavItem href="/settings#security" label="Security Config" active={hash === '#security'} isSubmenu />
            </div>
          )}
            </div>
          </div>
...
function NavItem({ href, label, active, isSubmenu = false }: { href: string; label: string; active: boolean; isSubmenu?: boolean }) {
  return (
    <a 
      href={href} 
      className={`flex items-center gap-3 px-4 py-1.5 transition-all rounded-md group ${
        isSubmenu ? 'text-[12px]' : 'text-[13px] font-medium'
      } ${
        active 
